...
function transformSchemaObjectWithComposition(schemaObject, options, fromAdditionalProperties = false) {
  ...
  if (Array.isArray(schemaObject.enum) && (!("type" in schemaObject) || schemaObject.type !== "object") && !("properties" in schemaObject)) {
    ...
    if (!hasAdditionalProperties || schemaObject.type === "string" && hasAdditionalProperties) {
      if (shouldTransformToTsEnum(options, schemaObject)) {
        ...
        const finalType2 = hasNull ? ts.tsUnion([ref, ts.NULL]) : ref;
        return applyAdditionalPropertiesToEnum(hasAdditionalProperties, finalType2, schemaObject);
      }
      ...
      const unionType = applyAdditionalPropertiesToEnum(hasAdditionalProperties, ts.tsUnion(enumType), schemaObject);
...
function transformSchemaObjectCore(schemaObject, options) {
  ...
  if ("properties" in schemaObject && schemaObject.properties && Object.keys(schemaObject.properties).length || "additionalProperties" in schemaObject && schemaObject.additionalProperties || "patternProperties" in schemaObject && schemaObject.patternProperties || "$defs" in schemaObject && schemaObject.$defs) {
    ...
    const patternProperties = hasKey(schemaObject, "patternProperties") ? schemaObject.patternProperties : void 0;
    const hasExplicitPatternProperties = typeof patternProperties === "object" && patternProperties !== null && Object.keys(patternProperties).length > 0;
    ...
    if (hasExplicitPatternProperties && patternProperties && typeof patternProperties === "object") {
      for (const [_, v] of utils.getEntries(
        patternProperties,
        options.ctx
      )) {
...
function applyAdditionalPropertiesToEnum(hasAdditionalProperties, unionType, schemaObject) {
  if (hasAdditionalProperties && schemaObject.type === "string") {
    const stringAndEmptyObject = ts.tsIntersection([ts.STRING, ts__default.factory.createTypeLiteralNode([])]);
    return ts.tsUnion([unionType, stringAndEmptyObject]);
  }
  return unionType;
}
